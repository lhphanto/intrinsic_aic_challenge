...
# ---------------------------------------------------------------------------
# Robot Policy Transformer  (image-conditioned, 2-token sequence)
# ---------------------------------------------------------------------------

class RobotPolicyTransformer(nn.Module):
    """Compact transformer for the RLPD robot policy.

    Sequence: [state_token, prev_action_token]  (length 2)
    No image / task / timestep tokens in the sequence — image information
    enters exclusively through AdaLN-Zero conditioning (512-dim vector).

    Only the prev_action_token position is read out and returned.
    """
    ...
    def __init__(
        ...
        ffn_dim: int = 512,
        dropout: float = 0.0,
        cond_dim: int = 3 * TOKEN_DIM,  # default: 3 cameras × TOKEN_DIM
    ):
        super().__init__()
    ...
    def forward(
        ...
        Args:
            ...
            cond:         (B, cond_dim) — image conditioning vector
        Returns:
            out: (B, d_model) — output at the prev_action_token position
        """
        x = torch.cat([state_token, action_token], dim=1)             # (B, 2, d_model)
...
# ---------------------------------------------------------------------------
# Robot Policy  (RLPD-compatible actor: Gaussian mean + log std output)
# ---------------------------------------------------------------------------
...
class RobotPolicy(nn.Module):
    """RLPD-compatible robot actor producing a Gaussian action distribution.

    Architecture:
        ImageConditioner (ResNet18 + global pool) → (B, 512) image condition
        state_encoder  robot_state → (B, 1, d_model)
        action_in_proj prev_action → (B, 1, d_model)
        RobotPolicyTransformer conditioned on image features via AdaLN-Zero
        output_head MLP → (B, 2 * action_dim)  split into [means | log_stds]
    ...
    def __init__(
        ...
        # cond_dim = one TOKEN_DIM embedding per camera, concatenated
        _num_cameras = self.image_conditioner.num_cameras
        _cond_dim    = _num_cameras * d_model
