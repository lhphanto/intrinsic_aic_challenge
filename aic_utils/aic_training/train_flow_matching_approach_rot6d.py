...
_STATE_NAMES = [
    ...
    "max_force_magnitude",
    "insertion_event",
    "teleop.approach_done", "teleop.approach_dist_x", "teleop.approach_dist_y", "teleop.approach_dist_z",
    "task.target_module", "task.port_name", "episode_number",
]
_SIDX = {name: i for i, name in enumerate(_STATE_NAMES)}

# tcp_pose.position.x/y/z + orientation.x/y/z/w (0:7)
# joint_positions.0..6 (19:26)
# wrench.force.x/y/z + torque.x/y/z (26:32)
...
_COL_APPROACH_DIST   = slice(_SIDX["teleop.approach_dist_x"], _SIDX["teleop.approach_dist_z"] + 1)
...
class AICLeRobotDataset(torch.utils.data.Dataset):
    ...
    def _load_state(self):
        ...
        dist_vecs  = state_arr[:, _COL_APPROACH_DIST]           # (N, 3)
        dist_norms = np.linalg.norm(dist_vecs, axis=1)          # (N,)
        over       = dist_norms > 1.0
        n_over     = int(over.sum())
        print(
            f"[AICLeRobotDataset] {n_over}/{len(state_arr)} frames have "
            f"dist_to_target > 1.0 — clamping to 1.0."
        )
        if n_over > 0:
            scale = np.where(over, 1.0 / dist_norms, 1.0)[:, None]  # (N, 1)
            state_arr[:, _COL_APPROACH_DIST] *= scale.astype(np.float32)
    ...
    def _build_windows(self, state_arr: np.ndarray) -> list[int]:
        ...
        for i in range(len(ep_starts) - 1):
            ...
            chunk = state_arr[s:e]
            valid = range((e - s) - self._window_len + 1)
            for w in list(valid)[1:-1]:
                windows.append(s + w)
        return windows
    ...
    def __getitem__(self, idx: int) -> dict:
        ...
        dist_xyz = torch.from_numpy(pred_state[:, _COL_APPROACH_DIST].astype(np.float32))  # (pred_horizon, 3)
        dist_to_target = dist_xyz.norm(dim=-1)                                              # (pred_horizon,)
...
def evaluate_val(
    ...
    """Average flow-matching loss over up to val_steps random batches."""
    policy.eval()
    total = 0.0
    n = 0
    with torch.no_grad():
        for batch in val_loader:
            total += flow_matching_loss(policy, batch, device).item()
            n += 1
            if n >= val_steps:
                break
    policy.train()
...
def train(args):
    ...
    if args.val_dataset_dirs:
        val_loader = build_dataloader(
            dataset_dirs = args.val_dataset_dirs,
            obs_horizon  = args.obs_horizon,
            pred_horizon = args.pred_horizon,
            action_dim   = args.action_dim,
            batch_size   = args.batch_size,
    ...
    for epoch in range(args.num_epochs):
        ...
        if val_loader is not None:
            val_loss = evaluate_val(policy, val_loader, device, args.val_steps)
            val_losses.append(val_loss)
            val_epoch_indices.append(epoch + 1)
            val_msg = f"  val_loss={val_loss:.4f}"

        print(f"Epoch {epoch + 1}/{args.num_epochs}  avg_loss={avg_loss:.4f}{val_msg}  time={elapsed_sec:.1f}s")
    ...
    axes[1].plot(range(1, len(epoch_losses) + 1), epoch_losses, marker="o", markersize=3, label="train")
    if val_losses:
        axes[1].plot(val_epoch_indices, val_losses, marker="s", markersize=3, label="val", color="orange")
        axes[1].legend()
...
def parse_args():
    ...
    p.add_argument("--val_steps", type=int, default=200,
                   help="Number of random batches to evaluate per epoch for validation loss")
    ...
    p.add_argument("--action_dim",   type=int, default=10,
                   help="Action dimension: 3 position + 6D rotation + dist_to_target = 10")
